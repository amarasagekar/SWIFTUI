...
struct PostCardView: View {
    ...
    var body: some View {
        ...
        .overlay(alignment: .topTrailing, content: {
            if post.userUID == userUID{
                Menu{
                    Button("Delete Post", role: .destructive, action:deletPost)
                }label: {
    ...
    func deletPost(){
        Task{
            do{
                if post.imageReferanceID != ""{
                   try await Storage.storage().reference().child("Post_Images").child(post.imageReferanceID).delete()
